...
export const Hexagon = (props: ComponentPropsWithoutRef<'svg'> & { size?: number; reverse?: boolean; duration?: number }) => {
  ...
  useLayoutEffect(() => {
    ...
    // Calculate scaled path length based on the size ratio
    const scaledPathLength = pathLength * (size / 82);
    setTotalPathLength(scaledPathLength);
  }, [size]); // Add size dependency
    ...
    <div className="inline-flex relative">
      ...
      {totalPathLength && (
        <motion.svg
          ...
          strokeWidth="6"
          xmlns="http://www.w3.org/2000/svg"
          className={twMerge("text-fuchsia-500/30 -rotate-6 absolute inset-0", className)}
          initial={{
            strokeDashoffset: 0
          }}
          animate={{
          ...
          style={{
            strokeDasharray: `${Math.min(500, totalPathLength * 0.3)} ${totalPathLength - Math.min(500, totalPathLength * 0.3)}`,
            height: size,
            width: size,
          }}
        >
